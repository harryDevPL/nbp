...
class CurrencyConfiguration {
    ...
    @Bean
    CurrencyRateProvider currencyRateProvider(CurrencyRepository repository, CurrencyFetcher fetcher) {
        return new SimpleCurrencyRateProvider(repository, fetcher);
    }

    @Bean
    CurrencyFetcher currencyFetcher(WebClient webClient, ClientProperties properties) {
